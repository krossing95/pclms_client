...
interface SearchHiddenBookingStates {
    keyword: string
    open: boolean
    message: string
    isErrorFree: boolean
...
interface SearchHiddenBookingsProps {
    paginate: (page: number, totalItem: number, totalPages: number) => void
}

const Search: React.FC<SearchHiddenBookingsProps> = ({ paginate }) => {
    const app = useAppSelector(state => state.appReducer.bookings)
    const dispatch = useAppDispatch()
    const stringMethod = StringMethods()
    const [states, setStates] = React.useState<SearchHiddenBookingStates>({
        keyword: '',
        open: false,
        message: '',
        isErrorFree: false,
    ...
    const searchHandler = async () => {
        ...
        try {
            const result = await search_hidden_bookings({ page: 1, keyword: word })
    ...
    return (
        <Dialog open={app.hasOpenedSearchBoxPrompt} PaperComponent={MovablePrompt} aria-labelledby="draggable-dialog-title">
            <DialogTitle className={styles.text} style={{ cursor: 'move' }} id="draggable-dialog-title">{"Find hidden bookings"}</DialogTitle>
            <MessageBox
                open={states.open}
            ...
            />
            <DialogContent>
                <Typography variant='body2'>{"Enter a keyword in the search box below to search for related hidden bookings"}</Typography>
